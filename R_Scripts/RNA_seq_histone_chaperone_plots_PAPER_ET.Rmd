...
AVERAGE_Alldata <- allDataTPM + 1
AVERAGE_Alldata <- log2(AVERAGE_Alldata)
...
#Alldata
meltedAllData <- melt(AVERAGE_Alldata, value.name = 'Count',
                             varnames=c('GeneID', 'Sample'))
```
...
```{r, i}
meltedAllData$Sample <- factor(meltedAllData$Sample, altorder)
```
...
box<-ggplot(meltedAllData, aes(x=Sample, y=Count)) +
